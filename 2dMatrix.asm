...
print_matrix:
    ...
    sw      $ra,                    0($sp)                                                                                              # Save return address
    sw      $s0,                    4($sp)                                                                                              # Save $s0 (matrix address)
    sw      $s1,                    8($sp)                                                                                              # Save $s1 (i = index, starts at 0)
    sw      $s2,                    12($sp)                                                                                             # Save $s2 (6, for Divide)
    sw      $s3,                    16($sp)                                                                                             # Save $s3 (36, total elements)
    sw      $t0,                    20($sp)                                                                                             # Save $t0 (temporary - current value)
    sw      $t1,                    24($sp)                                                                                             # Save $t1 (temporary)
    sw      $t2,                    28($sp)                                                                                             # Save $t2 (temporary)
    sw      $t3,                    32($sp)                                                                                             # Save $t3 (temporary)

    move    $s0,                    $a0                                                                                                 # s0 = matrix_address
    li      $s1,                    0                                                                                                   # i = 0 (start from first element)
    li      $s2,                    6                                                                                                   # s2 = 6 (columns per row)
    li      $s3,                    36                                                                                                  # s3 = 36 (total num of elements)

    # Print top border
    li      $a0,                    '+'                                                                                                 # Print corner
    li      $v0,                    SysPrintChar                                                                                                  # SysPrintChar
    syscall

    li      $t0,                    0                                                                                                   # Counter for top border
top_border_loop:
    bge     $t0,                    $s2,            top_border_done

   li      $a0,                    '-'                                                                                                 # Print horizontal line
    li      $v0,                    SysPrintChar
    syscall

    li      $a0,                    '-'                                                                                                 # Print another dash for width
    li      $v0,                    SysPrintChar
    syscall
...
print_col_loop:
    bge     $t1,                    $s2,            print_row_end                                                                       # If we've printed all columns, end the row

    # Load the current matrix element
    sll     $t0,                    $s1,            2                                                                                   # t0 = i * 4
    add     $t0,                    $t0,            $s0                                                                                 # t0 = address of current element
    lw      $t0,                    0($t0)                                                                                              # t0 = value at current element

    # Check if value is -2 (X) or -1 (O)
    li      $a0,                    ' '                                                                                                 # Start with a space for padding
    li      $v0,                    SysPrintChar
    syscall
...
print_onedig:
    move    $a0,                    $t0
    li      $v0,                    SysPrintInt                                                                                         # SysPrintInt
    syscall
    li      $a0,                    ' '                                                                                                 # Add trailing space for padding
    li      $v0,                    SysPrintChar
    syscall
    j       finish_cell
print_X:
...
finish_cell:
    ...
    # End cell with vertical bar
    li      $a0,                    '|'
    li      $v0,                    SysPrintChar
    syscall

    addi    $t1,                    $t1,            1                                                                                   # Increment column counter
    addi    $s1,                    $s1,            1                                                                                   # Increment element index
    j       print_col_loop
...
print_row_end:
    ...
    li      $t0,                    0                                                                                                   # Counter for row separator
row_sep_loop:
    bge     $t0,                    $s2,            row_sep_done
...
row_sep_done:
    ...
    bge     $s1,                    $s3,            print_matrix_done                                                                   # Check if we're done with all elements
    j       print_row_loop                                                                                                              # Otherwise, continue to next row
...
print_newline:
    ...
    li      $v0,                    SysPrintChar                                                                                                  # System call for print character
...
print_address:
    ...
    # Print newline
    li      $v0,                    SysPrintChar                                                                                                  # syscall for print_char
